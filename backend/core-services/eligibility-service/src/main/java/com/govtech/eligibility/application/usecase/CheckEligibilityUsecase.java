...
import com.govtech.eligibility.application.event.EligibilityEventService;
...
public class CheckEligibilityUsecase {
    ...
    private final EligibilityEventService eventService;
    ...
    public void check(
        ...
        log.info(
                "Starting eligibility calculation for subject={}",
                subject);
        ...
        try {
            ...
            log.debug(
                    "Profile successfully retrieved for subject={}",
                    subject);
            ...
            log.info(
                    "Persisted {} eligibility result(s) calculationId={}",
                    results.size(),
                    calculationId);

            eventService.publishEligibilityChecked(
                    subject,
                    results,
                    eventContext);

            log.info(
                    "Eligibility checked event created for subject={} calculationId={}",
                    subject,
                    calculationId);
            ...
            log.warn(
                    "Eligibility persistence failed for subject={}",
                    subject,
                    ex);
            ...
            log.error(
                    "Unable to retrieve profile for subject={}",
                    subject,
                    ex);
            ...
            log.error(
                    "Eligibility calculation failed for subject={}",
                    subject,
                    ex);

            throw new EligibilityException(
                    "Unable to calculate eligibility for subject "
                            + subject,
                    ex);
        }
    }

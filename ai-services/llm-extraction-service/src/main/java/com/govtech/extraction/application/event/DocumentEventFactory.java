...
public class DocumentEventFactory {
    ...
    public DocumentEventFactory(
            List<DocumentEventMapper<?>> mappers) {
    ...
    public Object create(
        ...
        DocumentEventMapper<?> mapper = mappers.get(
                document.type());

        if (mapper == null) {
            throw new IllegalArgumentException(
                    "No DocumentEventMapper registered for "
                            + document.type());
        }

        return mapper.toEvent(
                document,
                extractedData,
                model,
                eventContext);
